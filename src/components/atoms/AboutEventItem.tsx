...
interface AboutEventItemProps {
    ...
    placeName: string;
    date: string;
    queryPoint: QueryPointProps[];
    websiteLink: string;

}

const AboutEventItem: React.FC<AboutEventItemProps> = ({
                                                           image, eventName, eventDescription, placeName, date, queryPoint, websiteLink,
                                                       }: AboutEventItemProps) => {
    ...
    return (<section
            ...
            <section className="flex w-full items-center gap-2 justify-start">
                {image.map((img: string, index: number): any => (<img
            ...
            {/* querypoint */}
            {queryPoint.map((event: QueryPointProps, index: number) => (
