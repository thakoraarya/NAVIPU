...
export default function AdminForm() {
  ...
  const handleLocationFormSubmit = async () => {
    try {
      ...
      const response = await databases.createDocument(
        '651a64c780f4e31fb04c', // Replace with your Appwrite database ID
        collectionId,
        ID.unique(),
        {
  ...
  const handleUpload = () => {
    ...
    // You can use 'selectedFiles' to access the selected files
  ...
  return (<>
      ...
      <div className="flex m-1 p-1 gap-2 text-[#1B1C17]">
        <div className="w-1/2 h-fit p-4 border-2 rounded-lg bg-[#FBF9F1] shadow-md mt-4 md:mt-0">
          ...
          <br/>
          <br/>
          <button onClick={handleEventFormSubmit} className='bg-[#CAF234] rounded-3xl border-amber-900 border2 p-2'>Save Event</button>
          <p>{successMessage}</p>
        </div>
        <div className="w-1/2 h-fit p-4 border-2 rounded-lg bg-[#FBF9F1] shadow-md">
          ...
          />
          <button onClick={handleLocationFormSubmit} className='bg-[#CAF234] rounded-3xl border2 p-2'>Save Location</button>
          <p>{successMessage}</p>
        </div>
